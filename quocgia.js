...
const sliderIntervals = {};
  ...
  initSlider('.vietnam-slider');
  initSlider('.hanquoc-slider');
  initSlider('.trungquoc-slider');
  initSlider('.java-slider'); // phục hồi lại phần Java

  // Xử lý hiển thị thể loại theo URL
  const urlParams = new URLSearchParams(window.location.search);
  const genre = urlParams.get('genre') || urlParams.get('country'); // sua
  if (genre) {
    ...
    document.querySelector('.hanquoc-slider')?.classList.add('active'); // thể loại mặc định
  }
